...
#if 0
    int i;
    create_state_register(ST_TIMER,"neogeo_timer_count",1,&neogeo_timer_count,sizeof(double),REG_UINT32);
...
void my_timer(void)
{
	...
	init = 0;
	if (neogeo_region==REGION_EUROPE) {
    ...
    for (i = 0; i < MAX_TIMER; i++) {
	if (neogeo_timer_count >= timers[i].time && timers[i].del_it == 0) {
